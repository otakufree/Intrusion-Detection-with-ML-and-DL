# -*- coding: utf-8 -*-
"""Intrusion Detection System with ML&DL.py

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1rTCvJkrgKV1nchkh9IZMJPKrGOtv3K-Y
"""
...
from sklearn.metrics import roc_curve, auc, confusion_matrix, ConfusionMatrixDisplay
y_score_lin_svc = lin_svc.decision_function(x_test)
fpr, tpr, thresholds = roc_curve(y_test, y_score_lin_svc)
roc_auc = auc(fpr, tpr)
plt.figure(figsize=(8, 8))
plt.plot(fpr, tpr, color='blue', lw=2, label=f'ROC Curve (AUC = {roc_auc:.2f})')
plt.plot([0, 1], [0, 1], color='gray', linestyle='--', label='Random Guess')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curve - Linear SVC(LBasedImpl)')
plt.legend(loc="lower right")
plt.grid(True)
plt.tight_layout()
plt.show()
...
fpr, tpr, thresholds = roc_curve(y_test, y_pred_prob_tdt)
roc_auc = auc(fpr, tpr)
plt.figure(figsize=(8, 8))
plt.plot(fpr, tpr, color='blue', lw=2, label=f'ROC Curve (AUC = {roc_auc:.2f})')
plt.plot([0, 1], [0, 1], color='gray', linestyle='--', label='Random Guess')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curve - DecisionTreeClassifier (Full Depth)')
plt.legend(loc="lower right")
plt.grid(True)
plt.tight_layout()
plt.show()
...
ojectfig = plt.figure(figsize=(15,12))
...
from sklearn.metrics import roc_curve, auc, confusion_matrix, ConfusionMatrixDisplay
y_pred_prob_rrf = rrf.predict_proba(x_test_reduced)[:, 1]
fpr, tpr, thresholds = roc_curve(y_test_reduced, y_pred_prob_rrf)
roc_auc = auc(fpr, tpr)
plt.figure(figsize=(8, 8))
plt.plot(fpr, tpr, color='blue', lw=2, label=f'ROC Curve (AUC = {roc_auc:.2f})')
plt.plot([0, 1], [0, 1], color='gray', linestyle='--', label='Random Guess')
